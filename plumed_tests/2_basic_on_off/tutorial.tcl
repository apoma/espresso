...
#
# here plumed is run continuously
set i 0 
while { $i<$n_cycle } {
    integrate $n_steps
...
# here plumed is reset at every round 
# note that the history is lost at every step
# so, for example, metadynamics biases and work values are gone
set i 0 
while { $i<$n_cycle } {

set plumed_input [open "plumed.dat" "w"] 
puts  $plumed_input "d1: DISTANCE ATOMS=1,40" 
puts  $plumed_input "PRINT ARG=* STRIDE=10 FILE=COLVAR_$i" 
close $plumed_input
